# Custom Student (<10M params): TinyViT + TinyTextTransformer

This document provides a truly custom student architecture (no Hugging Face backbones) designed to learn from the repo's teacher via existing distillation losses.

Key constraint: the text embedding table can dominate parameters. If you keep a BERT-like vocab (~30k), keep the student text d_model small (e.g., 128) to stay under 10M params.

## Required output interface

Your custom student must return the same output keys used by the distillation losses:

- logits_modality, logits_location
- img_raw, txt_raw, img_proj, txt_proj

## Suggested architecture (stays <10M)

- Vision: tiny ViT (d_model=192, depth=4, heads=3)
- Text: tiny Transformer (d_model=128, depth=4, heads=4)
- Fusion: gated sum (tiny MLP)
...
### Custom student wrapper (repo-compatible output)
...
- Add a new model module (example: models/student_custom.py)
- In the trainer, instantiate StudentCustom when student.arch == "custom_tiny"
- Keep the output keys unchanged so distillation losses work without changes
